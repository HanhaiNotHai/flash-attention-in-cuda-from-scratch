...
__device__ void load_tile(const float *src,
    ...
    for (int idx = thread_id; idx < tile_size; idx += num_threads) {
        ...
        if (src_r < src_rows && src_c < src_cols) {
            shared_dst[r * tile_cols + c] = src[src_r * src_cols + src_c];
        } else {
            // zero padding for boundary tiles
            shared_dst[r * tile_cols + c] = 0.0f;
        }
    }
}
...
# Step 19 - tile_rowmax
__device__ void tile_rowmax(const float *s_tile,
                            float *row_max_out,
                            int tile_q,
                            int tile_k,
                            int thread_id,
...
# Step 20 - tile_exp
__device__ void tile_exp(float *s_tile, const float *row_max, int tile_q, int tile_k, int thread_id, int num_threads) {
    ...
    for (int idx = thread_id; idx < tile_size; idx += num_threads) {
        ...
        s_tile[r * tile_k + c] = expf(s_tile[r * tile_k + c] - row_max[r]);
    }
}
...
__device__ void accumulate_pv(const float *p_tile,
                              const float *v_tile,
                              float *out_acc,
                              int tile_q,
                              int tile_k,
    ...
    int out_size = tile_q * head_dim;

    for (int idx = thread_id; idx < out_size; idx += num_threads) {
        int r = idx / head_dim;
        int d = idx % head_dim;
        ...
        for (int c = 0; c < tile_k; c++) {
            sum += p_tile[r * tile_k + c] * v_tile[c * head_dim + d];
        }
